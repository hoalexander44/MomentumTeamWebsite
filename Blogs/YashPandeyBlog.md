...
I think one area we can improve on is mapping out the progress we made on the game (from a programmer's point of view). I think it'd be more efficient overall of the heirarchy and the processes of the code that's working to run the game was laid out better/explained more. It was difficult to simply "get the player to move" and I had to jump through some coding hurdles for that one to work
